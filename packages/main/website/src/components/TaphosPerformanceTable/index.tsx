...
try {
  benchmarkData = require("../../data/taphos-benchmark.json");
} catch {
  benchmarkData = null;
}
...
const taphosWeights = functionWeights.modules.taphos as Record<string, { category: CategoryName; reason: string }>;
...
// Extract utility name from scenario name (e.g., "flatten\ndeep" -> "flatten")
...
function getScenarioWeight(scenarioName: string): { category: CategoryName; weight: number; reason: string } | null {
  ...
  const weightInfo = taphosWeights[utilName];
...
export function TaphosBenchmarkDataMissing(): React.ReactElement {
  return (
    <div className={styles.errorContainer}>
      <h3>⚠️ Benchmark data not found</h3>
      <p>
        The benchmark results file <code>taphos-benchmark.json</code> is missing.
      </p>
      <p>Generate it by running:</p>
      <pre>
        <code>pnpm doc:generate:taphos:benchmarks-results</code>
      </pre>
    </div>
  );
}
...
export function TaphosGeneratedDate(): React.ReactElement {
  if (!benchmarkData) return <span>N/A</span>;
  return <span>{formatDate(benchmarkData.generatedAt)}</span>;
}

// TL;DR component
export function TaphosPerfTLDR(): React.ReactElement {
  if (!benchmarkData) return <TaphosBenchmarkDataMissing />;

  const { scenarios } = benchmarkData;
  
  // Calculate native wins
  let nativeWins = 0;
  let taphosWins = 0;
  let total = 0;
  
  for (const scenario of scenarios) {
    const winner = scenario.results.find(r => r.isFastest);
    if (!winner) continue;
    total++;
    if (winner.library === "native") nativeWins++;
    if (winner.library === "taphos") taphosWins++;
  }

  return (
    <p className={styles.tldr}>
      <strong>Native JavaScript wins {Math.round((nativeWins / total) * 100)}% of benchmarks.</strong> That's the point — 
      Taphos helps you migrate from Lodash, but the real goal is native JS. 
      Among libraries, Taphos is competitive with {taphosWins} wins, beating es-toolkit/compat on critical functions.
    </p>
  );
}
...
const libraryDescriptions: Record<string, string> = {
  taphos: "Pithos utility module (modern ES2020+)",
  "native": "Native JavaScript equivalent (baseline comparison)",
...
export function TaphosVersionsTable(): React.ReactElement {
  if (!benchmarkData) return <TaphosBenchmarkDataMissing />;
  ...
  const versionEntries = Object.entries(versions)
    ...
    .sort(([a], [b]) => {
      if (a === "taphos") return -1;
      if (b === "taphos") return 1;
...
export function TaphosBenchmarkResultsTable(): React.ReactElement {
  if (!benchmarkData) return <TaphosBenchmarkDataMissing />;

  const { scenarios, libraries } = benchmarkData;

  // Sort libraries: native first, then taphos, then alphabetically
...
export function TaphosPerformanceSummary(): React.ReactElement {
  if (!benchmarkData) return <TaphosBenchmarkDataMissing />;
...
export function TaphosDetailedStats(): React.ReactElement {
  if (!benchmarkData) return <TaphosBenchmarkDataMissing />;
...
export function TaphosWeightedSummary(): React.ReactElement {
  if (!benchmarkData) return <TaphosBenchmarkDataMissing />;
