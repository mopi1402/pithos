...
export function lowerCase(str: string | null | undefined): string {
  ...
  if (len <= CHARCODE_THRESHOLD) {
    // Check for non-ASCII in the same pass as word splitting
    let result = "";
    let i = 0;
    ...
    while (i < len) {
      ...
      if (c > 127) {
        ...
        return (str.match(RE_UNICODE_WORDS) as RegExpMatchArray).map(w => w.toLowerCase()).join(" ");
      }
      ...
      // Stryker disable next-line ConditionalExpression: after separator skip, only digits (48-57), uppercase (65-90), and lowercase (97-122) remain — c >= 65 && true still routes lowercase to the else branch which handles them identically
      if (c >= 65 && c <= 90) {
        const uStart = i;
        i++;
        ...
        if (i < len && str.charCodeAt(i) >= 97 && str.charCodeAt(i) <= 122) {
          if (i - uStart > 1) {
            if (result) result += " ";
            result += str.slice(uStart, i - 1).toLowerCase();
            const wStart = i - 1;
            i++;
            ...
            result += " " + str.slice(wStart, i).toLowerCase();
          } else {
            i++;
            // Stryker disable next-line EqualityOperator: i <= len would read charCodeAt(len) → NaN, failing all comparisons and exiting naturally
            while (i < len) {
            ...
            if (result) result += " ";
            result += str.slice(uStart, i).toLowerCase();
          }
        } else {
          if (result) result += " ";
          result += str.slice(uStart, i).toLowerCase();
