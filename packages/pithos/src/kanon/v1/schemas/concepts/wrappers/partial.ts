 ...
 * @since 1.1.0
 */
export class PithosPartial<T extends Record<string, PithosType>>
  implements
    PithosType<
    ...
    >
{
  ...
  safeParse(
    ...
    const result: { [K in keyof T]?: T[K]["_output"] } = {};
    const issues: any[] = [];
    ...
    for (const [key, schema] of Object.entries(this.shape)) {
      ...
      // Si la clé n'existe pas, on ne l'ajoute pas (champ optionnel)
    }
